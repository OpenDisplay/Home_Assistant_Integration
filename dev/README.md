...
| Key | Why |
|---|---|
| `http: !include http_port.yaml` | `http_port.yaml` is generated by `dev/run.sh` on every start (`server_port: $HA_PORT`) — without this, `hass` always binds its default 8123 regardless of `HA_PORT`, so a non-default `HA_PORT` run would probe a different port than the one `hass` actually bound (adversarial review round 3, finding 2 — reproduced live, see below). |
