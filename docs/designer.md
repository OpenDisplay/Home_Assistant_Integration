...
| `resolveAsset` | GETs this integration's own asset endpoint (below) for a font or image the designer couldn't resolve locally, and hands back the bytes |
| `hostOwnsAssets` | `{ hint: '…' }` (designer 3.4.0) — turns off designer-local uploads and replaces the Content tab's upload instructions with a sentence naming where a file actually needs to live so `resolveAsset` above can find it |
...
The current pins are the designer at **3.4.3** and `js-yaml` at **4.1.0**
...
the published `odl-drawcustom-designer.d.ts` was byte-identical across all
four releases. 3.4.0 grew the `.d.ts` for the first time in this series —
purely additively, a new `MountOptions.hostOwnsAssets` key, nothing removed
or reshaped — and this integration adopted it the same round (see "The
asset endpoint" and the `hostOwnsAssets` table row above). 3.4.1 fixed the
embedded panel header falling back to a commit SHA instead of the library's
own `APP_VERSION`; not independently verifiable without a browser, but
expected to show the designer version now. 3.4.2 is the designer's own
release-pipeline unification (its `.d.ts` only reworded a couple of doc
comments to say "the release pipeline" instead of naming an internal script)
— no embed-surface change. 3.4.3 is the first release under that repo's new
"every push to main bumps at least a patch" rule, applied to a docs-only
change; verified directly (tarball diff, not assumed): the published
`odl-drawcustom-designer.d.ts` is byte-identical to 3.4.2's, and the only
line that changed in the built bundle is its own embedded build-identity
constant (commit SHA and version string) — no behavioural change, no
embed-surface change. Every type and member the wrapper reads or writes
has been re-checked against the 3.4.3 `.d.ts`
(`mount`, all five `MountHandle` methods used, all eleven `MountOptions`
keys now passed — `hostOwnsAssets` newly among them, `HostTarget`, all ten
`HostDisplaySpec` keys written, `HostActionContext`/`HostPreviewContext` and
their `display`/`render` members, `HostAction` including
`severity: 'caution'`, `DesignerStatus`'s `yamlValid`/`yamlErrorSummary`,
and `EmbedTheme`) — zero misses, no other wrapper change needed.
