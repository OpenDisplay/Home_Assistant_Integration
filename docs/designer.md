...
The current pins are the designer at **3.2.0** and `js-yaml` at **4.1.0**
...
that quietly ages. 3.0.0 → 3.2.0 changed the bundle only: the published
`odl-drawcustom-designer.d.ts` is byte-identical across those releases, and
every type and member the wrapper reads or writes was re-checked against it
