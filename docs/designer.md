...
The current pins are the designer at **3.2.1** and `js-yaml` at **4.1.0**
...
that quietly ages. 3.0.0 → 3.2.0 → 3.2.1 changed the bundle only: the
published `odl-drawcustom-designer.d.ts` is byte-identical across all three
releases, and every type and member the wrapper reads or writes has been
re-checked against it at each bump
